  ...
  const getData = useCallback(
    async function loadData() {
      const response = await fetch(
        `${import.meta.env.PUBLIC_LOCATIONS_ROWS_URL}`,
        {
          headers: {
    ...
    <>
      <div className='flex justify-center w-full mb-6'>
        <input
          ...
          className='py-2 px-3 rouded-md bg-slate-100'
          type='text'
        ...
        <table className='table w-full'>
          ...
          <thead>
            <tr>
              {headers?.map((header) => (
                <th
                  key={header.id}
                  className='px-8 w-[80px] m-auto text-yellow-500 text-2xl text-center'
                >
                  {header.name}
                </th>
              ))}
          ...
          <tbody>
            ...
            {data?.map((result, index) => (
              <tr
                key={index}
                className='mt-4 text-xl text-slate-600 hover:bg-slate-600 hover:text-slate-100'
              >
                <td className='text-center w-[40px]'>{result.Name}</td>
                <td className='text-center max-w-10 overflow-x-auto hover:text-white font-bold'>
                  <a href={result.Location} target='_blank'>
                    Map
                  </a>
                </td>
